...
class BERTERTrainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        #for batch_idx, (tcr_tokenized, antigen_tokenized, target) in enumerate(self.data_loader):
        for batch_idx, (tcr_a_tokenized,tcr_b_tokenized, antigen_tokenized, hla_tokenized, target) in enumerate(self.data_loader):

            tcr_a_tokenized = {k: v.to(self.device) for k, v in tcr_a_tokenized.items()}
            tcr_b_tokenized = {k: v.to(self.device) for k, v in tcr_b_tokenized.items()}
            antigen_tokenized = {k: v.to(self.device) for k, v in antigen_tokenized.items()}
            hla_tokenized = {k: v.to(self.device) for k, v in hla_tokenized.items()}
            target = target.to(self.device)
            
            self.optimizer.zero_grad()
            output = self.model(tcr_a_tokenized, tcr_b_tokenized, antigen_tokenized, hla_tokenized)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (tcr_a_tokenized, tcr_b_tokenized, antigen_tokenized, hla_tokenized, target) in enumerate(self.valid_data_loader):
                tcr_a_tokenized = {k: v.to(self.device) for k, v in tcr_a_tokenized.items()}
                tcr_b_tokenized = {k: v.to(self.device) for k, v in tcr_b_tokenized.items()}
                antigen_tokenized = {k: v.to(self.device) for k, v in antigen_tokenized.items()}
                hla_tokenized = {k: v.to(self.device) for k, v in hla_tokenized.items()}
...
###
                ...
                y_pred = y_pred.cpu().detach().numpy()
                y_true = target.cpu().detach().numpy()
                result_dict['y_pred'].extend(y_pred)
                result_dict['y_true'].extend(y_true)
    ...
    def test(self, tcr_tokenizer, antigen_tokenizer, hla_tokenizer):
        self.model.eval()
        result_dict = {'tcr_a': [],'tcr_b':[], 'antigen':[], 'hla': [],
                       'y_true': [], 'y_pred': []}
        with torch.no_grad():
            for batch_idx, (tcr_a_tokenized,tcr_b_tokenized, antigen_tokenized, hla_tokenized, target) in enumerate(self.test_data_loader):
                tcr_a_tokenized = {k: v.to(self.device) for k, v in tcr_a_tokenized.items()}
                tcr_b_tokenized = {k: v.to(self.device) for k, v in tcr_b_tokenized.items()}
                antigen_tokenized = {k: v.to(self.device) for k, v in antigen_tokenized.items()}
                hla_tokenized = {k: v.to(self.device) for k, v in hla_tokenized.items()}
                ...
                tcr_a = tcr_tokenizer.batch_decode(tcr_a_tokenized['input_ids'],
                                                        skip_special_tokens=True)
                tcr_a = [s.replace(" ", "") for s in tcr_a]

                tcr_b = tcr_tokenizer.batch_decode(tcr_b_tokenized['input_ids'],
                                                        skip_special_tokens=True)
                tcr_b = [s.replace(" ", "") for s in tcr_b]

                antigen = antigen_tokenizer.batch_decode(antigen_tokenized['input_ids'],
                                                        skip_special_tokens=True)
                antigen = [s.replace(" ", "") for s in antigen]

                hla = hla_tokenizer.batch_decode(hla_tokenized['input_ids'],
                                                        skip_special_tokens=True)
                hla = [s.replace(" ", "") for s in hla]

                result_dict['tcr_a'].append(tcr_a)
                result_dict['tcr_b'].append(tcr_b)
