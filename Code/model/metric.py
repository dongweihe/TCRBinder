...
def macro_auc01(y_pred, y_true, peptides):
    """
    Compute Macro AUC0.1 (Partial AUC up to FPR=0.1 using McClish standardization).
    Calculated independently for each peptide and then averaged.

    Args:
        y_pred (array-like): Predicted scores (probabilities or logits).
        y_true (array-like): True binary labels (0 or 1).
        peptides (array-like): Peptide/Antigen IDs corresponding to each sample.

    Returns:
        float: The macro-averaged AUC0.1 score.
    """
    y_pred = np.array(y_pred).flatten()
    y_true = np.array(y_true).flatten()
    peptides = np.array(peptides).flatten()
    ...
    for pep in unique_peptides:
        ...
        # AUC is defined only if there are both positive and negative samples for this peptide
        if len(np.unique(sub_true)) < 2:
            continue

        # max_fpr=0.1 calculates the partial AUC up to 0.1 FPR.
        # sklearn automatically applies McClish standardization to range [0.5, 1.0]
        # when max_fpr is specified.
        try:
            p_auc = metrics.roc_auc_score(sub_true, sub_pred, max_fpr=0.1)
            auc_scores.append(p_auc)
        except ValueError:
            # Handle edge cases where numerical issues might occur
            continue
